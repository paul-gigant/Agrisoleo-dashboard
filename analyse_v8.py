...
def extraction_of_configuration(input_file, xshape_field_data ,yshape_field_data):
    ...
    size_field = (input_file.iloc[3,1],input_file.iloc[3,2])
    size_module = (input_file.iloc[12,1],input_file.iloc[12,2])
    number_module = (input_file.iloc[14,1],input_file.iloc[14,2])
    gap_module = (input_file.iloc[15,1],input_file.iloc[15,2])
    resolution = input_file.iloc[30,1]
    azimut = input_file.iloc[8,1]
    start_date = input_file.iloc[31,1]
    end_date = input_file.iloc[32,1]

    nb_days_temp = end_date-start_date
    nb_days = nb_days_temp.total_seconds()/(3600*24)

    if input_file.loc[21,1] =='OUI':
        tracker = True
    elif input_file.loc[21,1] =='NON':
        tracker = False
    if input_file.loc[24,1] =='OUI':
        backtracking = True
    elif input_file.loc[24,1] =='NON':
        backtracking = False
    ...
    offset_SaaS = (input_file.iloc[13,1],input_file.iloc[13,2]) #data from SaaS corresponding to the middle of the module 
    ...
    coordonne_site = (input_file.iloc[6,1],input_file.iloc[6,2])
    hauteur = input_file.iloc[17,1]
    opacite = input_file.iloc[18,1]
    orientation_static = input_file.iloc[22,1]
    orientation_maximale = input_file.iloc[23,1]
    frequence = input_file.iloc[33,1]
